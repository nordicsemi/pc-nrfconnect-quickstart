...
const VerifyStep = ({
    ...
    const failed = useAppSelector(getVerifyFailed);
    ...
    const verify = useCallback(() => {
        if (!deviceConnected) {
            dispatch(setVerifyFailed('No development kit connected.'));
            setVerifying(false);
            return;
        }
        ...
        runVerification(commands, path, mode)
            ...
            .catch(e => {
                logger.error(e);
                dispatch(setVerifyFailed('Failed to verify device.'));
            })
            .finally(() => {
                setVerifying(false);
            });
...
const VerifyConfigLayer = ({ settings }: { settings: ChoiceSettings[] }) => {
    ...
    if (choiceSettings && path) {
        return (
            <VerifyStep
                ...
                commands={choiceSettings.commands}
            />
        );
    }
...
export default (config: { settings: ChoiceSettings[] }) => ({
    name: 'Verify',
